...
- **다른 화면과의 차이**
  - 예매 탭(`booking/index.tsx`)에도 검색이 있지만 **방식이 다르다.** 거기는 카탈로그가
    50건이라 검색창을 상시 노출하고, 입력하면 목록 자체가 결과로 바뀐다. 여기는 티켓이
    몇 장뿐이라 아이콘을 눌러야 열리고, 목록을 거르는 대신 맞는 카드를 맨 앞으로 올린다.
  - 여권 화면(`passport.tsx`) 헤더는 검색 아이콘만 있고 탭해도 아무 동작이 없다(미구현).

---

## 12. 예매 목록 검색창

예매 탭 상단, 카테고리 탭 위에 항상 떠 있는 검색창.

- **코드**: `app/(tabs)/booking/index.tsx`
- **왜 상시 노출인가**: 카탈로그가 50건이라 카테고리만으로는 찾기 어렵다.
  보딩패스 탭처럼 아이콘 뒤에 숨기면 있는 줄도 모른다.
- **찾는 범위**: 제목 + 장소(`venue_name`). 공연장 이름만 기억나는 경우가 흔하다.
- **카테고리와의 관계**: 검색 중에는 **카테고리를 무시하고 전체에서 찾는다.**
  "레베카"를 찾으려고 뮤지컬 탭을 먼저 골라야 한다면 검색을 쓸 이유가 없다.
- **검색 중 탭 처리**: 카테고리 탭을 감추고 `전체에서 검색 · n건` 요약 줄로 바꾼다.
  탭이 남아 있으면 "이 카테고리 안에서만 찾는 중"으로 오해하게 된다.
  요약 줄의 세로 여백은 탭 줄과 같게 맞춰서, 검색을 켜고 끌 때 목록이 위아래로 튀지 않는다.
- **모양**: 테두리 1px `border-hairline`, radius 10(`TextField`와 같은 값),
  왼쪽 `search-outline` 아이콘, 입력 중에는 오른쪽에 `close-circle`(지우기).
